...
class info(communicate):
    ...
    def prepare(self):
        "signal strenght and phone number"
        self._send_cmd('AT+CUSD=1', return_data=True)
        operator = self._send_cmd("AT+CSPN?", return_data=True).decode()
        if operator == 'MTS RUS':
            self._send_cmd('AT+CUSD=1,"*111*0887#",15', return_data=True, t=3)
        if operator == 'beeline':
            self._send_cmd('AT+CUSD=1,"*110*10#",15', return_data=True, t=6)
